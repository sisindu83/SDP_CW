...
$("#pmfld").on('click', '.edit-row-rec', function() {
    ...
    var defval = $(this).closest("tr").find("td:eq(4) input[type='hidden']").val();
    defval = parseFloat(defval);

    var type = $(this).closest("tr").find("td:eq(3) input[type='hidden']").val();

    $('#fldtype').val(type);

    if ($(this).closest("tr").find("td:eq(5) input[type='hidden']").val() == 1) {
        $("#editmode").prop("checked", true);

    }
    $('#defval').val(defval.toFixed(2));
...
function delete_pfld_record(rec_id, tbl_raw) {
    ...
    $.ajax({
        url: "/payment/other_plan_field/delete",
        type: 'POST',
