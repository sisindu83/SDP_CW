...
$(document).ready(function() {
    $("#pmpln").on("input", function() {
        ...
        discount_val = parseFloat($('#scholdiscountval').val());


        blp = req_val - (prepaid_val + total + discount_val);
        blp = blp.toFixed(2);
        $('#balance_val').val(blp)



        $('#div_totdis').text(numberWithCommas(blp));
...
function change_date() {
    ...
    if (delregid > 0) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                change_fh_date(delregid, chdate)
            }
        })
...
function change_fh_date(delregid, chdate) {
    var _token = $("input[name=_token]").val();
    $.ajax({
        url: "/payment/students/change_date",
        type: 'POST',
        data: { _token: _token, regid: delregid, h_date: chdate },
        success: function(data) {
